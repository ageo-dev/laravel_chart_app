...
<div class="row mt50">
    ...
    <div class="col-sm-12 mt50 col-md-8">
        <p>Operating from&nbsp;&nbsp;&nbsp;{{\Carbon\Carbon::parse($user->start_date)->format(' m / d  / Y ')}}</p>
        <p>Show another month</p>
        <form id="chart_form" action="{{route('logSearch')}}" method="post">
            {{ csrf_field() }} 
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                @forelse ($logs as $log)
                    @isset($updateDate)
                        @php
                            $log_date = \Carbon\Carbon::parse($log->show_datetime)->format('Y-m-d');
                            $log_show_date = \Carbon\Carbon::parse($log->show_datetime)->format('Y-m-d ')
                            .str_pad(floor($appConfig->update_hour), 2, 0, STR_PAD_LEFT)
                            .':'
                            .str_pad(floor($appConfig->update_minute), 2, 0, STR_PAD_LEFT)
                        @endphp
                        @if ($log_date != \Carbon\Carbon::Parse($updateDate)->format('Y-m-d') ||
                            \Carbon\Carbon::now() >=  $updateDate)
                            @php
                                if(isset($oldFund)){
